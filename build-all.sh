...
if ! docker build -f base.Dockerfile -t static-tool-base .; then
...
for tool in "${TOOLS[@]}"; do
  ...
  if [ ! -f "${tool}.Dockerfile" ] && [ ! -f "${tool}"*.Dockerfile ]; then
    echo "Warning: Dockerfile for $tool not found, skipping..."
    continue
  fi
  
  # Build the Docker image
  if ! docker build -f ${tool}.Dockerfile -t ${tool}-static .; then
